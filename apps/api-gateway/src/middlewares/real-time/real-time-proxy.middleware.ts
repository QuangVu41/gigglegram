...
@Injectable()
export class RealTimeProxyMiddleware implements NestMiddleware {
  private readonly logger = new Logger(RealTimeProxyMiddleware.name);

  use = createProxyMiddleware({
    target: process.env.REAL_TIME_SERVICE_URL!,
    changeOrigin: true,
    on: {
      proxyReq: this.onProxyReq.bind(this),
      error: this.onError.bind(this),
    },
    pathRewrite: buildPathRewriteConfig('real-time'),
  });
  ...
  onError(
    ...
    this.logger.error('Error connecting to Real Time Service.', error);
