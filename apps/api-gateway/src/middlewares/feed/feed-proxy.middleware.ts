...
@Injectable()
export class FeedProxyMiddleware implements NestMiddleware {
  private readonly logger = new Logger(FeedProxyMiddleware.name);
  ...
  use = createProxyMiddleware({
    ...
    pathRewrite: buildPathRewriteConfig('feed'),
  });
  ...
  onError(
    ...
    this.logger.error('Error connecting to Feed Service.', error);
