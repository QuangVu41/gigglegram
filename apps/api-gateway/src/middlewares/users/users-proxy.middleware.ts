...
export class UsersProxyMiddleware implements NestMiddleware {
  private readonly logger = new Logger(UsersProxyMiddleware.name);
  ...
  use = createProxyMiddleware({
    ...
    pathRewrite: buildPathRewriteConfig('users'),
  });
  ...
  onError(
    ...
    this.logger.error('Error connecting to Users Service.', error);
