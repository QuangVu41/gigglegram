...
interface MediaPreviewStepperProps {
  ...
  showNavigateBtns?: boolean;
}

const MediaPreviewStepper = ({
  mediaFiles,
  setSelectedFileIndex,
  onVideoRef,
  showNavigateBtns = true,
}: MediaPreviewStepperProps) => {
    ...
    >
      <StepperPanel className="text-sm relative">
        {mediaFiles.map((file, idx) => (
          ...
          >
            <Card className="relative flex-1 items-center overflow-y-auto shadow-none no-scrollbar bg-background border-0 p-0">
              {file.file.type.startsWith("image/") && (
                <ImagePreview file={file} />
              )}
              {file.file.type.startsWith("video/") && (
                <VideoPreview file={file} onVideoRef={onVideoRef} />
              )}
            </Card>
          </StepperContent>
        ))}
