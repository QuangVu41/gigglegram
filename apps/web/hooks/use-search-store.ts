...
export type SearchItemData = typeof users.$inferSelect | typeof hashtags.$inferSelect;
export type SearchData = SearchItemData[];

export const isUserSearchItem = (item: SearchItemData): item is typeof users.$inferSelect => {
...
interface SearchStore {
  ...
  isLoading: boolean;
  fetchData: (query: Partial<FindManyQueryDto> & Record<string, string | number | string[]>) => Promise<void>;
...
export const useSearchStore = create<SearchStore>((set) => ({
  ...
  recentSearches: [],
  isLoading: false,
  fetchData: async (query: Partial<FindManyQueryDto> & Record<string, string | number | string[]>) => {
    set({ isLoading: true });
  ...
  saveRecentSearch: (data: SearchItemData) => {
    const recentSearches = JSON.parse(localStorage.getItem("recentSearches") || "[]") as SearchData;
    const incomingKey = getSearchItemKey(data);

    if (!recentSearches.some((item) => getSearchItemKey(item) === incomingKey)) {
  ...
  removeRecentSearch: (key: string) => {
    const recentSearches = JSON.parse(localStorage.getItem("recentSearches") || "[]") as SearchData;
    const updatedSearches = recentSearches.filter((item) => getSearchItemKey(item) !== key);
